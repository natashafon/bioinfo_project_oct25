...
#' TASK 10 - DATA.TABLE VERSION
#'
#' @description
#' This function maps ATAC-seq peaks to gene bodies using `data.table`:
#' it sets keys on genomic coordinates, performs an interval overlap join
#' between peaks and genes, computes overlap length in base pairs,
#' counts peaks per gene, and returns the top 20 genes ranked by total
#' overlapped base pairs.
#'
...
#' TASK 10 - DATA.FRAME VERSION
#'
#' @description
#' This function maps ATAC-seq peaks to genes using base R operations:
#' it compares genomic intervals per chromosome, calculates overlap
#' length in base pairs, counts overlapping peaks per gene, and returns
#' the top 20 genes with the highest total overlap.
#'
#' @param peaks_file Path to CSV file of ATAC-seq peaks (columns: chr, start, end, score, etc.)
#' @param genes_file Path to CSV file of gene annotation (columns: chr, gene_start, gene_end, gene)
