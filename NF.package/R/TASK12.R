...
#' TASK 12 - DATA.TABLE VERSION
#'
#' @description
#' This function combines two cohort sample tables safely using `data.table`:
#' it aligns columns across cohorts, merges them, orders by cohort, condition,
#' and sample ID, joins the combined data with bulk counts, identifies the top
#' 100 most variable genes, and computes per-cohort and per-condition mean counts.
#'
...
#' TASK 12 - DATA.FRAME VERSION
#'
#' @description
#' This function combines two cohort sample tables safely using base R:
#' it aligns and merges columns, orders samples, joins with bulk counts,
#' selects the top 100 most variable genes, and calculates per-cohort and
#' per-condition mean expression levels.
#'
#' @param cohortA_file Path to cohortA_samples.csv
#' @param cohortB_file Path to cohortB_samples.csv
#' @param counts_file Path to bulk_counts_long.csv (columns: gene, sample_id, count)
